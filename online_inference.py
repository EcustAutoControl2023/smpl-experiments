...
try:
    for i in range(len(model_names)):
        ...
        observations_list, actions_list, rewards_list = env.evalute_algorithms(
            algorithms, num_episodes=num_episodes, to_plt=False, plot_dir=save_dir
        )
        ...
        results_dict = env.report_rewards(
            rewards_list,
            algo_names=env.algorithms_to_algo_names(algorithms),
            save_dir=save_dir,
        )
        results_csv.append(
            [
                algo_name,
                results_dict[f"{algo_name}_on_episodes_reward_mean"],
                results_dict[f"{algo_name}_on_episodes_reward_std"],
                results_dict[f"{algo_name}_all_reward_mean"],
                results_dict[f"{algo_name}_all_reward_std"],
            ]
        )
    ...
    print(e)
with codecs.open(
    os.path.join(plt_dir, "total_results_dict.csv"), "w+", encoding="utf-8"
) as fp:
