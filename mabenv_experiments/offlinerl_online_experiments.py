...
for seed in seeds:
    ...
    for algo_name in algo_names:
        ...
        if online_training:
            ...
            wandb_run = wandb.init(
                reinit=True, project=project_title, name=acutal_dir, dir=logdir
            )
        ...
        if online_training:
            ...
            def online_saving_callback(algo, epoch, total_step):
                mean_env_ret = d3rlpy.metrics.evaluate_on_environment(
                    env, n_trials=10, epsilon=0.0
                )(algo)
                global ONLINE_PREV_EVALUATE_ON_ENVIRONMENT_SCORER
            ...
            for epoch, metrics in curr_algo.fitter(
                ...
                if (
                    ...
                    curr_algo.save_model(
                        os.path.join(
                            acutal_dir, "best_continuous_action_diff_scorer.pt"
                        )
                    )
            if evaluate_on_environment:
                shutil.copyfile(
