...
if __name__ == "__main__":
    ...
    parser.add_argument("--num_of_initial_state", type=int, default=10000)
    parser.add_argument(
    ...
    if args.initial_states_loc == "":
        if not args.evenly_spread_initial_states:
            ...
            initial_states = env.evenly_spread_initial_states(args.val_per_state)
            num_of_initial_state = len(initial_states)
        initial_states_loc = os.path.join(
            dataset_location, f"{env_name}/initial_states"
        )
        mzutils.mkdir_p(initial_states_loc)
        initial_states_loc = os.path.join(
            initial_states_loc, f"{num_of_initial_state}.npy"
        )
    ...
    dataset_loc = os.path.join(
        dataset_loc, f"{num_of_initial_state}_normalize={normalize}.pkl"
    )
